...
{ 
   ...
   ////lấy data loại sản phẩm
    public ArrayList<LoaisanphamDTO> readLoaianphamDAO() {
        ...
        try {
               String sSQL="SELECT tbl_loaisanpham.id_loaisanpham,tbl_loaisanpham.ten_lsp"
            ...
            if(rs!=null)
            {
                while(rs.next())
                {
                    ...
                    lspDTO.getIdLoaiSanphamDTO(rs.getInt("id_loaisanpham"));
                    lspDTO.getTenLoaiSanphamDTO(rs.getString("ten_lsp"));
                    list.add(lspDTO);
                }
            }
        } 
